...
async def verify_token(token: str):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT user_id, expires_at, used FROM email_verifications WHERE token = $1",
            token
        )
        ...
        if expires < datetime.now(timezone.utc):
            return None
        await conn.execute("UPDATE email_verifications SET used = TRUE WHERE token = $1", token)
...
async def send_verification_email(email: str, name: str, token: str, base_url: str):
    ...
    <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px">
      ...
      <p style="color:#4a5568;line-height:1.6">
        ...
        Click below to verify your email and activate your account. Link expires in 24 hours.
      </p>
      <a href="{verify_url}"
      ...
      <p style="color:#718096;font-size:12px;margin-top:24px">
        If you did not create a Project Elevate account, ignore this email.
      </p>
    </div>
    """
