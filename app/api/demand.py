...
async def run_reddit_ingestion_endpoint(
    ...
    """
    Scrape healthcare subreddits for clinical pain points and store as hospital needs.
    Uses Reddit public JSON endpoint — no API key required.
    Set dry_run=true to preview without storing.
    """
    from app.ingestion.reddit_pipeline import run_reddit_ingestion
    result = await run_reddit_ingestion(
        max_per_subreddit=max_per_subreddit,
        dry_run=dry_run,
    )
    return {
        "posts_fetched":     result.posts_fetched,
        "pain_points_found": result.pain_points_found,
