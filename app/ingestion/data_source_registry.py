...
DATA_SOURCES = [
    ...
    DataSource(
        ...
        name="cBioPortal — Cancer Genomics (GENIE/TCGA)",
        category="epidemiology",
        commercial_status="ALLOWED",
        license_type="CC BY 4.0 — commercial use YES",
        api_endpoint="https://www.cbioportal.org/api",
        connector_file="ingestion/connectors/cbioportal.py",
        unique_data="Real patient-level biomarker frequencies from 175K+ sequenced tumors (AACR GENIE + TCGA). KRAS G12C=13.1% NSCLC, HER2-low=54% HR+ breast, BCMA=96% myeloma — measured from actual sequencing, not estimates. Enables precision oncology TAM calculation anchored in real patient data.",
        attribution='Source: "AACR Project GENIE Consortium. Cancer Discov. 2017;7(8):818-831. cbioportal.org CC BY 4.0."',
        restrictions="Attribution required. Cannot claim AACR endorses product.",
        rate_limit="None documented; polite use recommended",
        requires_key=False, key_source=None,
    ),

    DataSource(
        source_id="dailymed",
        name="DailyMed — FDA-Approved Drug Labeling",
        category="regulatory",
        commercial_status="CLEAR",
        license_type="US Public Domain (NLM/NIH)",
        api_endpoint="https://dailymed.nlm.nih.gov/dailymed/services/v2/",
        connector_file="ingestion/connectors/dailymed.py",
        unique_data="Complete FDA-approved structured product labels (SPL): indications, Black Box warnings, dosing regimen, clinical trial data, contraindications, pharmacokinetics, NDC codes. More detailed than openFDA label endpoint. Black Box warnings = REMS/market access signal.",
        attribution='Source: "DailyMed. National Library of Medicine. dailymed.nlm.nih.gov"',
        restrictions="None (public domain).",
        rate_limit="None documented",
        requires_key=False, key_source=None,
    ),
    ...
    DataSource(
        ...
        name="Human Protein Atlas — Tissue Expression",
        category="regulatory",
        commercial_status="ALLOWED",
        license_type="CC BY 4.0 — commercial use YES",
        api_endpoint="https://www.proteinatlas.org/api/",
        connector_file="ingestion/connectors/human_protein_atlas.py",
        unique_data="Protein expression by tissue (normal + cancer) from IHC and RNA-seq. Therapeutic window assessment: target in tumor vs normal tissue. Subcellular location (membrane/secreted/nuclear) for modality recommendation. Pre-loaded profiles for EGFR, HER2, KRAS, PD-L1, SMN1, BCMA, HBB.",
        attribution='Source: "Human Protein Atlas. proteinatlas.org. CC BY 4.0."',
        restrictions="Attribution required.",
        rate_limit="None documented",
        requires_key=False, key_source=None,
    ),
    ...
    DataSource(
        source_id="gtex",
        name="GTEx — Tissue-Specific Gene Expression (Normal)",
        category="regulatory",
        commercial_status="CLEAR",
        license_type="NIH open-access tier — unrestricted (dbGaP phs000424)",
        api_endpoint="https://gtexportal.org/api/v2/",
        connector_file="ingestion/connectors/gtex.py",
        unique_data="RNA-seq expression for 20K+ genes across 54 normal human tissues. eQTL data. Distinguishes ubiquitous (toxicity risk) vs tissue-restricted (clean therapeutic window) expression. Complements HPA (protein in disease) with RNA in healthy tissue.",
        attribution='Source: "GTEx Consortium. Science. 2020;369:1318-1330. gtexportal.org"',
        restrictions="Open-access aggregated data unrestricted. Individual genotypes need dbGaP approval (not needed for expression lookups).",
        rate_limit="None documented on open-access API",
        requires_key=False, key_source=None,
    ),

    DataSource(
        source_id="nih_gard",
        name="NIH GARD — Rare Disease Information",
        category="epidemiology",
        commercial_status="CLEAR",
        license_type="US Public Domain (NIH/NCATS)",
        api_endpoint="https://rarediseases.info.nih.gov/api/",
        connector_file="ingestion/connectors/nih_gard.py",
        unique_data="6,800+ rare diseases with US-standard naming, inheritance patterns, prevalence category, related genes, OMIM/Orphanet/ICD-10 cross-references. Bridges Orphanet (EU naming) with US FDA/NIH regulatory terminology. FDA orphan designation applications cite GARD directly.",
        attribution='Source: "NIH GARD. rarediseases.info.nih.gov. National Center for Advancing Translational Sciences."',
        restrictions="None (public domain).",
        rate_limit="None documented",
        requires_key=False, key_source=None,
    ),
    ...
    DataSource(
        source_id="lens_patents",
        name="Lens.org — Global Patent Database",
        category="ip",
        commercial_status="ALLOWED",
        license_type="CC0 for patent metadata; Lens API ToS permits commercial use",
        api_endpoint="https://api.lens.org/patent/search",
        connector_file="ingestion/connectors/lens_patents.py",
        unique_data="120M+ global patents across USPTO + EPO + WIPO + CNIPA (vs PatentsView's US-only). PCT applications show global IP strategy before US filing. Patent family size = IP protection depth. Assignee changes = BD deal signal. Pre-loaded patent profiles for pembrolizumab, semaglutide, adalimumab, osimertinib.",
        attribution='Source: "Lens.org (Cambia). CC0 patent metadata. lens.org"',
        restrictions="API key required (free). Attribution for patent metadata.",
        rate_limit="10 req/sec with API key",
        requires_key=True, key_source="https://www.lens.org/lens/user/subscriptions",
    ),
    ...
    DataSource(
        ...
        category="regulatory",
        commercial_status="ALLOWED",
        license_type="Open Government Licence – Canada (OGL-C) — commercial use YES",
        api_endpoint="https://health-products.canada.ca/api/drug/",
        connector_file="ingestion/connectors/health_canada.py",
        unique_data="Canadian drug approval status, DIN, therapeutic class, market status. Fills Canada gap (12th largest pharma market, ~$35B USD/yr, ~3% global pharma spend). Canada approval typically 12-18 months after FDA.",
        attribution='Source: "Health Canada Drug Product Database. health-products.canada.ca. OGL-C."',
        restrictions="Attribution required.",
        rate_limit="None documented",
        requires_key=False, key_source=None,
    ),
    ...
    DataSource(
        source_id="cms_formulary",
        name="CMS Medicare Advantage & ACA Formulary Data",
        category="economic",
        commercial_status="CLEAR",
        license_type="US Public Domain",
        api_endpoint="https://data.cms.gov/resource/qkpc-4bfn.json",
        connector_file="ingestion/connectors/cms_formulary.py",
        unique_data="Formulary tier placement (1-5), Prior Authorization flags, Step Therapy requirements, Quantity Limits by plan. Coverage rate = % of plans covering the drug. MASSIVE TAM impact: Wegovy 28% coverage → formulary-adjusted TAM is 28% of theoretical. Pre-loaded for 9 key drugs.",
        attribution='Source: "CMS Medicare Advantage Formulary Reference File. cms.gov. Public domain."',
        restrictions="Cannot imply CMS endorsement.",
        rate_limit="Socrata standard",
        requires_key=False, key_source="https://data.cms.gov/developer",
    ),
    ...
    DataSource(
        ...
        name="County Health Rankings & CDC Social Vulnerability Index",
        category="epidemiology",
        commercial_status="ALLOWED",
        license_type="CC BY 4.0 (CHR) + US Public Domain (CDC SVI)",
        api_endpoint="https://services3.arcgis.com/ZvidGQkLzkAIsvpq/arcgis/rest/services/CDC_Social_Vulnerability_Index_2022/FeatureServer/0/query",
        connector_file="ingestion/connectors/county_health_rankings.py",
        unique_data="County-level obesity (41% MS vs 24% CO), diabetes rates, uninsured %, mental health provider density, PCP supply. CDC SVI: composite vulnerability by census tract. State-level disease burden hotspots for geographic launch targeting. Pre-loaded 20 states.",
        attribution='Source: "County Health Rankings 2024, CC BY 4.0, countyhealthrankings.org. CDC SVI 2022, US Public Domain."',
        restrictions="CHR: attribution required. CDC SVI: public domain.",
        rate_limit="ArcGIS REST: no documented limit",
        requires_key=False, key_source=None,
    ),

    # NEWLY BLOCKED — must not use commercially
    DataSource(
        source_id="dartmouth_atlas_blocked",
        name="Dartmouth Atlas — BLOCKED (Non-Commercial Only)",
        category="economic",
        commercial_status="BLOCKED",
        license_type="Dartmouth custom license — explicitly non-commercial",
        api_endpoint=None,
        connector_file=None,
        unique_data="HRR/HSA-level procedure rates. BLOCKED — use CMS Geographic Variation (data.cms.gov) as public domain alternative.",
        attribution="N/A — BLOCKED",
        restrictions="Explicitly non-commercial per Dartmouth Atlas license. Use CMS Geographic Variation instead.",
        rate_limit="N/A",
        requires_key=False, key_source=None,
    ),

    DataSource(
        source_id="wipo_blocked",
        name="WIPO PATENTSCOPE Free Tier — BLOCKED (Non-Commercial)",
        category="ip",
        commercial_status="BLOCKED",
        license_type="WIPO Terms — free tier is non-commercial only",
        api_endpoint="https://patentscope.wipo.int/search/en/rest/v1/",
        connector_file=None,
        unique_data="PCT patent applications. BLOCKED free tier — use Lens.org (CC0) instead for global patent data commercially.",
        attribution="N/A — BLOCKED (free tier)",
        restrictions="Free API tier explicitly non-commercial. Lens.org (CC0) covers same PCT data commercially.",
        rate_limit="N/A",
        requires_key=False, key_source=None,
    ),
